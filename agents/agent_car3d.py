...
class AgentCar3D:
    ...
    def verifier(self, idx, plan: Waypoint, init_set):
        ...
        dynamics = "dryvr_dynamics/NN_car3d_TR_noNN"
        time_horizon = plan.time_bound
        variables_list = ['x', 'y', 'z', 'theta']
        ...
        res = verify(
            ...
            initset_resolution = [0.5, 0.5, 0.01, 0.1]
        )
    ...
    def execute_plan(self):
        ...
        while i < len(self.waypoint_list):
            ...
            res = self.verifier(
                ...
                plan = current_plan, 
                init_set = [
                    [curr_init_set[0]-1, curr_init_set[1]-1, curr_init_set[2] - 0.01, curr_init_set[3]-0.01],
                    [curr_init_set[0]+1, curr_init_set[1]+1, curr_init_set[2] + 0.01, curr_init_set[3]+0.01]
                ]
            )
            ...
            curr_init_set = [trace[-1][1], trace[-1][2], trace[-1][3], trace[-1][4]]
...
if __name__ == "__main__":
    ...
    z_init = 0
    theta_init = np.random.uniform(np.pi/2-0.5,np.pi/2+0.5)
    car = AgentCar3D(0, wp, [x_init,y_init,z_init,theta_init])
