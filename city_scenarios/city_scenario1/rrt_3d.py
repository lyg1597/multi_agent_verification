...
class RRT3D:
    ...
    def __init__(self, start, goal_list, obstacle_list, rand_area, color = 'b',
                 ...
                 path_resolution=1, goal_sample_rate=0, max_iter= 5000):
        """
        Setting Parameter
        ...
        self.min_rand_z = rand_area[4]
        self.max_rand_z = rand_area[5]
    ...
    def planning(self, animation=True):
        ...
        while i < self.max_iter:
            ...
            in_reached,_ = self.in_reached_goal(new_node.x, new_node.y, new_node.z, self.goal_list)
            if in_reached:
                continue
            ...
            if safe:
                print("goal reached", i, self.node_list[-1].x, self.node_list[-1].y, self.node_list[-1].z)
    ...
    def draw_graph(self, rnd=None):
        ...
        for (A,b) in self.obstacle_list:
            # vtc = pc.extreme(pc.Polytope(A = A[:,0:2], b = b[:,:]))
            poly_2d = pc.projection(pc.Polytope(A = A, b = b), dim = [1,2])
            vtc = ppm.compute_polytope_vertices(poly_2d.A,poly_2d.b)
    ...
    def in_goal(x, y, z, goal_list):
        ...
        safe = False
        for i in range(len(goal_list)):
            if check_in_polytope(np.array([x,y,z]),(goal_list[i].A,goal_list[i].b)):
                safe = True
                break
        return safe,i

    @staticmethod
    def in_reached_goal(x, y, z, goal_list):
        # TODO: Change this function to handle a list of goals.
        safe = False
        for i in range(len(goal_list)):
            if check_in_polytope(np.array([x,y,z]),(goal_list[i].A,goal_list[i].b)) and goal_list[i].reached:
                safe = True
                break
        return safe,i
