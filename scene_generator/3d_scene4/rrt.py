...
class RRT:
    ...
    class Edge:
        ...
        def __init__(self, node, parent, color = 'b'):
            self.start = [parent.x,parent.y]
            self.end = [node.x,node.y]
            self.color = color
    ...
    def __init__(self, start, goal_list, obstacle_list, rand_area, color = 'b',
        ...
        self.goal_list = [self.Goal(A,b) for [A,b] in goal_list]
    ...
    def planning(self, animation=True):
        ...
        while i < self.max_iter:
            ...
            new_node = self.steer(nearest_node, rnd_node, self.expand_dis, self.expand_angle)

            in_reached,_ = self.in_reached_goal(new_node.x, new_node.y, self.goal_list)
            if in_reached:
                continue
            elif self.check_collision(new_node, self.obstacle_list) and self.check_orientation(new_node):
                self.node_list.append(new_node)
                i += 1          


            safe, idx = self.in_goal(self.node_list[-1].x, self.node_list[-1].y,self.goal_list)
            if safe:
                print("goal reached", i, self.node_list[-1].x, self.node_list[-1].y)
    ...
    def check_orientation(self, node):
        if node.parent is None or node.parent.parent is None:
            return True
        else:
            seg1 = ([node.parent.x,node.parent.y],[node.x,node.y])
            orientation1 = np.arctan2(seg1[1][1] - seg1[0][1],seg1[1][0] - seg1[0][0])
            seg2 = ([node.parent.parent.x,node.parent.parent.y],[node.parent.x,node.parent.y])
            orientation2 = np.arctan2(seg2[1][1] - seg2[0][1],seg2[1][0] - seg2[0][0])
            diff_angle = abs(orientation1 - orientation2)
            if diff_angle <= 1.58 or diff_angle >= 4.71:
                return True
            else:
                return False

    def steer(self, from_node, to_node, extend_length=[float("inf")], extend_angle = [float("inf")]):
        ...
        theta = min(extend_angle, key = absDiffFunc)
    ...
    def generate_edges(self):
        ...
        for node in self.reach_node_list:
            while node.parent is not None:
                if (node.parent.x, node.parent.y, node.x, node.y) in edge_dict:
                    break
                parent_node = node.parent
                edge = self.Edge(node,parent_node, self.color)
                edge_dict[(node.parent.x, node.parent.y, node.x, node.y)] = edge
                node = parent_node
        return edge_dict
    ...
    def get_random_node(self):
        ...
        while True:
            ...
            in_reached,_ = self.in_reached_goal(x,y,self.goal_list)
            if not in_reached and self.check_collision(self.Node(x,y),self.obstacle_list):
                break
        rnd = self.Node(x,y)
        # else:  # goal point sampling
            # i = random.randint(0,len(self.end)-1)
            # rnd = self.Node(self.end[i].x, self.end[i].y)
        return rnd
    ...
    @staticmethod
    def in_goal(x, y, goal_list):
        # TODO: Change this function to handle a list of goals.
        safe = False
        for i in range(len(goal_list)):
            if check_in_polytope(np.array([x,y]),(goal_list[i].A,goal_list[i].b)):
                safe = True
                break
        return safe,i

    @staticmethod
    def in_reached_goal(x, y, goal_list):
        # TODO: Change this function to handle a list of goals.
        safe = False
        for i in range(len(goal_list)):
            if check_in_polytope(np.array([x,y]),(goal_list[i].A,goal_list[i].b)) and goal_list[i].reached:
                safe = True
                break
        return safe,i
