...
class RRT:
    ...
    def __init__(self, start, goal, obstacle_list, rand_area, dim = 2,
        ...
        self.start = self.Node(start_pt)
        self.end = self.Node(end_pt)
    ...
    def planning(self, animation=True):
        ...
        for i in range(self.max_iter):
            ...
            if self.in_goal(self.node_list[-1].x, self.node_list[-1].y,(self.goal[0],self.goal[1])):
        ...
        # path = self.generate_path(self.reach_node_list)
    ...
    def get_random_node(self):
        if random.randint(0, 100) > self.goal_sample_rate:
            ...
            for i in range(self.dimension):
                tmp_state.append(random.uniform(self.min_rand, self.max_rand))
            rnd = self.Node(tmp_state)
    ...
    @staticmethod
    def in_goal(x, y, goal):
        safe = check_in_polytope(np.array([x,y]),goal)
        return safe
