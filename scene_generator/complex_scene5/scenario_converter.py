...
def convert_to_json(fn = './scene.json',obs = [], init = [], goal = [], edges = {}, trans_res = 0.01):
    f = open(fn,'w+')
    data = {
        "reachability_engine":"",
    ...
    for g in goal:
        ...
        g_A = np.array(g[0])
        g_b = np.array(g[1])
        poly = pc.Polytope(g_A,g_b)
        
        lower = []
        upper = []
        for i in range(g_A.shape[0]//2):
            tmp1 = float(g_A[i*2,i]*g_b[i*2,0])
            tmp2 = float(g_A[i*2+1,i]*g_b[i*2+1,0])
            if tmp1<tmp2:
                lower.append(tmp1)
                upper.append(tmp2)
            else:
                lower.append(tmp2)
                upper.append(tmp1)
        g_set = g_set + [[lower,upper]]
        goal_set_list.append(g_set)
    ...
    for obstecle in obs:
        ...
        vertices = pc.extreme(poly).tolist()
...
def run_planner(obs, init, goal, search_area, color = 'b'):
    ...
    rrt = RRT(start=[A_start, b_start],
                ...
                obstacle_list=obs,
                expand_dis=[3], max_iter = 60000, color = color)
...
if __name__ == "__main__":
    ...
    for A,b in obs:
        ppm.polygon.plot_polygon(ppm.duality.compute_polytope_vertices(A,b-2), color = 'red')
    ...
    convert_to_json(fn = output_fn, obs = obs, init = init, goal = goal, edges = edge_list,trans_res=0.25)