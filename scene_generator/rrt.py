...
class RRT:
    ...
    def __init__(self, start, goal, obstacle_list, rand_area,
                 expand_dis=[3.0], path_resolution=0.01, goal_sample_rate=0, max_iter= 5000):
        """
        Setting Parameter
    ...
    def generate_edges(self):
        ...
        for node in self.reach_node_list:
            while node.parent is not None:
                if (node.x,node.y) in edge_dict:
                    break
                parent_node = node.parent
                edge = self.Edge(node,parent_node)
                edge_dict[(node.x,node.y)] = edge
                node = parent_node
        return edge_dict
