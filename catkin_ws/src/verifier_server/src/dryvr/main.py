...
from dryvr.src.backend.reachabilityengine import ReachabilityEngine
from dryvr.src.frontend.scenario import Scenario
from dryvr.src.frontend.plotter import plot_rtsegment_and_traces
import numpy as np

def get_tube(scenario_file_name):
    cur_scenario: Scenario = Scenario(scenario_file_name = scenario_file_name)
...
def get_tube_input(dryvr_input):
    cur_scenario:Scenario = Scenario(scenario = dryvr_input)
    reachtube_segment, simulation_traces= ReachabilityEngine.get_reachtube_segment(cur_scenario.initial_set)
    # plot_rtsegment_and_traces(reachtube_segment, cur_scenario.initial_set.training_traces)
    #print("dryvr initial set has a center of ", cur_scenario.initial_set.initial_center, " and radii",
    #      cur_scenario.initial_set.initial_radii)
